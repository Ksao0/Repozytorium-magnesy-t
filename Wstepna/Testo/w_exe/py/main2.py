...
try:
    ...
    url = "https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/Odbiorca.exe"
            ...
            with open("Styl.txt", "r", encoding='utf-8') as plik:
                ...
                if os.path.isfile(f"styl_{styl}.css"):
                    ...
                    try:
                        ...
                        url = f"https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/Style/styl_{styl}.css"
            ...
            try:
                ...
                try:
                    try:
                        ...
                        url = "https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/Style/styl_szarość.css"
...
class AutoStartManager:
    ...
    def run(self):
        try:
            ...
            if main2_folder:
                print(
                    "Znaleziono folder zawierający main2.exe i folder rei:", main2_folder)

                # Ścieżka do pobrania pliku Automa.exe
                automa_url = "https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/Automa.exe"
...
def download_icon():
    try:
        # Zmień na właściwy adres URL pliku .ico
        url = "https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/icon.ico"
...
def sprawdzanie_nowych_aktualizacji():
    ...
    while True:
        try:
            ...
            try:
                url = 'https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/version.txt'
                response = requests.get(url)
...
class OknoAktualizacji(QWidget):
    ...
    def okno_instalator(self):
        ...
        if instalator_sesja == 0:
            ...
            def otworz():
                try:
                    # ścieżka do pliku Instalator.exe w bieżącym folderze
                    path = os.path.join(os.getcwd(), "Instalator.exe")

                    # usuń plik Instalator.exe, jeśli istnieje
                    if os.path.exists(path):
                        os.remove(path)
                    # print("Usunięto plik Instalator.exe")

                    # pobierz plik main.exe z repozytorium
                    url = "https://raw.githubusercontent.com/Ksao0/Repozytorium-magnesy-t/main/Wstepna/Testo/w_exe/Instalator.exe"
                    urllib.request.urlretrieve(url, path)
                    # print("Zastąpiono plik Instalator.exe")

                    subprocess.run(['python', 'Instalator.exe'])
...
''', zakladka)
    ...
    def automa(self):
        if messagebox.askokcancel("Autostart", 'Ta opcja doda aktualizator do autostartu, możesz to cofnąć w każdej chwili\nW niektórych przypadkach możesz zobaczyć okno terminalu na killka sekund\nJeśli program się zamknie - coś poszło nie tak'):
            ...
            # Kolorowe instrukcje
            print(f"{Fore.LIGHTBLACK_EX}Aby wyłączyć - usuń plik {Fore.LIGHTYELLOW_EX}startup.exe{Style.RESET_ALL}{Fore.LIGHTBLACK_EX} z tej ścieżki:\n{sciezka_do_kasowania}{Fore.LIGHTBLACK_EX}\nPamiętaj o uzupełnieniu nazwy użytkownika ({Fore.BLUE}{getpass.getuser()}{Style.RESET_ALL}{Fore.LIGHTBLACK_EX})\nEwentualnie usuń {Fore.LIGHTYELLOW_EX} startup.exe{Style.RESET_ALL}{Fore.LIGHTBLACK_EX} z autostartu w menedżerze zadań\n")
        else:
    ...
    def otworz_odbiorca(self):
        def w_nowym_watku():
            try:
                ...
                if os.path.isfile("Odbiorca.exe"):
                    subprocess.run(['python', 'Odbiorca.exe'])
                else:
                    toaster = Powiadomienia()
                    toaster.powiadomienie_jednorazowe(
