    ...
    if version_local != 'BRAK DANYCH':
        ...
        if version_online_lines[0] == version_local_lines[0] and version_online_lines[1] == "Status: B7":
            ...
            if messagebox.askokcancel("Aktualizacja", message):
                ...
                exit()
        if version_online_lines[0] == version_local_lines[0] and (version_online_lines[1] == "Status: Poprawki B7 zakończone" or version_online_lines[1] == "Status: Poprawki B7 (zakończone)") and version_local_lines[1] != version_online_lines[1]:
            Aktualizacja = ["python", "WEW.py"]
            subprocess.run(Aktualizacja)
...
def otworz_okno_wybor():
    ...
    label_informacja = tk.Label(
        okno_wyborowe, text="Ręcznie zaktualizuje program oraz/nZapisz informacje o aktualizacji do historii\nDo wyboru są dwie stale aktualizowane wersje:\nWstępna i Stabilna")
